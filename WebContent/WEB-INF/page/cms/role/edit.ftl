...
function memberSubmitForm(parentIndex){
	...
	if(s){
		...
		$.ajax({
			...
			dataType:"json",
			data:datas,
			url:"${base}/admin/role/update.htm",
...
<form class="form-horizontal" id="roleAddForm">
	    	<div class="form-group">
			    ...
			    <div class="col-xs-5">
			        <input type="text" class="form-control" placeholder="用户名" name="name" id="name" value="${role.name}">
			...
			<div class="form-group">
			    ...
			    <div class="col-xs-5">
			    	<select  class="form-control" name="status" id="status">
			    		[#list status as s]
			    			<option value="${s}" [#if s==role.status]selected=selected[/#if]>${s.description}</option>
			    		[/#list]
			    	</select>
			    </div>
			</div>
