var _all_menu_index_index_index=0;
$(function() {
    function t(t) {
        var e = 0;
    ...
    function n() {
    	var ttttt = $('.J_iframe[data-id="' + $(this).attr("href") + '"]');
    	if(ttttt.length>0){
    		ttttt.html("数据加载中......")
            $.ajax({
            	type:"post",
        ...
        if ($(".J_menuTab").each(function() {
            ...
            var r = $('<div class="J_iframe" id="iframe' + a + '"  data-id="' + t + '" style="width:100%;height:100%;overflow-y:auto;overflow-x:visible;position:relative;">数据加载中......</div>');
            $(".J_mainContent").find("div.J_iframe").hide().parents(".J_mainContent").append(r);
            menuA.find("i:first").on("click", c)
            menuA.find("i:last").on("click", s)
    ...
    function c() {
        var t = $('.J_iframe[data-id="' + $(this).parents().data("id") + '"]');
        t.html("数据加载中......")
        $.ajax({
        	type:"post",
        	url:$(this).parents().data("id"),
        	dataType:"html",
        	success:function(data){
        		t.html(data)
        	},error:function(){
    ...
    _all_menu_index_index_index=$(".J_menuItem").length;
